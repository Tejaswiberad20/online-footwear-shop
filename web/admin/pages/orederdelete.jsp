...
<%
    // Get the ID from the request (passed via the query string, for example, "deleteuser.jsp?oid=1")
    String oid = request.getParameter("oid");

    // Debug: Output the ID value to check what's being passed
    out.println("<script type=\"text/javascript\">");
    out.println("console.log('Received ID: " + oid + "');"); // Log the ID value to the browser console
    out.println("</script>");

    // Check if the ID is valid (it's numeric and not null/empty)
    if (oid == null || oid.trim().isEmpty() || !oid.matches("\\d+")) {
        out.println("<script type=\"text/javascript\">");
    ...
    try {
        ...
        String checkSql = "SELECT COUNT(*) FROM order WHERE oid = ?";
        pst = con.prepareStatement(checkSql);
        pst.setInt(1, Integer.parseInt(oid)); // Set the ID parameter for the query
        ...
        if (rs.next() && rs.getInt(1) > 0) {
            ...
            pst.setInt(1, Integer.parseInt(oid)); // Set the ID parameter for deletion
